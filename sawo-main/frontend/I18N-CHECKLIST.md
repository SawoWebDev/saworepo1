...
| `/sauna/heaters/floor` | ✅ | ✅ | ⬜ | **Wired 2026-09-04** — `Floor.jsx` now calls `useLocaleT("sauna")`/`useLocaleT("common")`/`useLocalizedPath()`, same `Stone.jsx`/`WallMounted.jsx` pattern as the other heater sub-pages: SEO title/description/path/`hreflangAlternates`, hero title/subtitle/alt/explore-button, intro heading/desc, filter-pill "All" + search placeholder/clear (via `common.json`'s `catalogFilter.*`), empty state, product-card `<Link>` (was a bare `/products/${slug}`, not locale-prefixed), "View All Heaters" link (was a bare `menuPaths.heaters`, same locale-dropping bug class fixed elsewhere in this file), Why-choose section, brochure link, and `PromoBanner` props. This page previously had **zero** i18n wiring at all — unlike Stone/WallMounted/Tower, which turned out to already be partially done when reached. New `floorPage` namespace added to `sauna.json` for en+zh. **`fi` added 2026-09-04** — 14/14 keys, 0 gap verified via key-diff and `npm run i18n:manifest`. Zero hardcoded-English hits on both audit regexes, `useLocaleT` count 3. |
| `/sauna/heaters/combi` | ✅ | ✅ | ⬜ | **Wired 2026-09-04** — `Combi.jsx`, same pattern/scope as `floor` above (also had zero prior i18n wiring). New `combiPage` namespace in `sauna.json` (en+zh only). Fixed the same locale-dropping product-card `<Link>` and "View All Heaters" link bugs. **`fi` added 2026-09-04** — 15/15 keys, 0 gap. Zero hardcoded-English hits on both audit regexes, `useLocaleT` count 3. |
| `/sauna/heaters/dragonfire` | ✅ | ✅ | ⬜ | **Wired 2026-09-04** — `Dragonfire.jsx`, same pattern/scope as `floor` above (also had zero prior i18n wiring). New `dragonfirePage` namespace in `sauna.json` (en+zh only). Fixed the same locale-dropping product-card `<Link>` and "View All Heaters" link bugs. **`fi` added 2026-09-04** — 15/15 keys, 0 gap. Zero hardcoded-English hits on both audit regexes, `useLocaleT` count 3. `CI=true npx react-scripts build` confirmed zero errors after all 3 of these pages (floor/combi/dragonfire) were wired together, completing i18n wiring for all 6 heater-series sub-pages (stone/tower/wall-mounted done earlier, these 3 done in this pass). |
| `/sauna/accessories/accessory-sets` | ✅ | ✅ | ⬜ | **Wired 2026-09-04** — `AccessorySets.jsx` now calls `useLocaleT("sauna")`/`useLocaleT("common")`/`useLocalizedPath()` (following the `Stone.jsx`/`SaunaAccessories.jsx` pattern): SEO title/description (+`hreflangAlternates`), hero title/subtitle/alt/brochure button, intro heading/desc, filter-pill label(s) (via a `groups` key map, falls back to the raw group name via i18next `defaultValue` so business-logic `FIXED_ORDER`/`GROUP_KEYWORDS` matching stays untouched) + search placeholder/clear/no-results/results-count (via `common.json`'s `catalogFilter.*`), empty/no-match states, product-card grid group headings, "View All Accessories" link (reused the existing `accessoriesPage.viewAll` key from `sauna.json` rather than duplicating it), `WhyChooseSawo` props, and `PromoBanner` props. New `accessorySetsPage` namespace added to `sauna.json` for en+zh. **`fi` added 2026-09-04** — full `accessorySetsPage` section written, reusing established `accessoriesPage` Finnish accessory-catalog terminology; key-parity verified 0 gap. Zero hardcoded-English hits on the audit regexes. Split out of the old `/sauna/accessories/*` catch-all row — see the row below for what's still untouched. |
| `/sauna/accessories/benches-floor-tiles` | ✅ | ✅ | ⬜ | **Wired 2026-09-04** — `BenchesFloorTiles.jsx`, same pattern/scope as `accessory-sets` above. New `benchesFloorTilesPage` namespace in `sauna.json` (en+zh, **`fi` added 2026-09-04** — 0 key-parity gap). Group filter pills (Benches/Wooden Floor Mats/Cloth Hangers) translated via the same `groups` key-map + `defaultValue` fallback approach. Zero hardcoded-English hits on the audit regexes. |
| `/sauna/accessories/clocks-sandtimers` | ✅ | ✅ | ⬜ | **Wired 2026-09-04** — `ClocksSandtimers.jsx`, same pattern/scope as `accessory-sets` above. New `clocksSandtimersPage` namespace in `sauna.json` (en+zh, **`fi` added 2026-09-04** — 0 key-parity gap). Group filter pills (Clocks/Sand Timers) translated via the same `groups` key-map + `defaultValue` fallback approach. Zero hardcoded-English hits on the audit regexes. |
| `/sauna/accessories/doors-handles` | ✅ | ✅ | ⬜ | **Wired 2026-09-04** — `DoorsHandles.jsx`, same pattern/scope as `accessory-sets` above. New `doorsHandlesPage` namespace in `sauna.json` (en+zh, **`fi` added 2026-09-04** — 0 key-parity gap). Group filter pills (Sauna Doors/Handles) translated via the same `groups` key-map + `defaultValue` fallback approach — the underlying `GROUP_KEYWORDS`/name-matching logic (which relies on the untranslated English "Door"/"Door Handle" substrings) was left completely untouched, only the *displayed* pill/heading text goes through `t()`. Zero hardcoded-English hits on the audit regexes. |
| `/sauna/accessories/headrests-backrests` | ✅ | ✅ | ⬜ | **Wired 2026-09-04** — `HeadrestsBackrests.jsx`, same pattern/scope as `accessory-sets` above. New `headrestsBackrestsPage` namespace in `sauna.json` (en+zh only at wiring time). Group filter pills (Headrests/Backrests) translated via the same `groups` key-map + `defaultValue` fallback approach. Zero hardcoded-English hits on the audit regexes. `CI=true npx react-scripts build` confirmed zero errors after all 5 of these pages were wired together. `npm run i18n:manifest` confirms `sauna: zh translated`, 0 gap. **`fi` added 2026-09-04** for this row plus the other 8 accessory sub-pages and the 2 rooms sub-pages below — 0 key-parity gap, `npm run i18n:manifest` reconfirms `sauna: fi translated`, 0 gap. |
| `/sauna/accessories/kivistone` | ✅ | ✅ | ⬜ | **Wired 2026-09-04** — `Kivistone.jsx` now calls `useLocaleT("sauna")`/`useLocaleT("common")`/`useLocalizedPath()` (same `Stone.jsx`/`SaunaAccessories.jsx` pattern as the 5-page batch above, done concurrently by a parallel workstream in this same repo): SEO title/description/path/`hreflangAlternates`, hero title/subtitle/alt/brochure button, intro heading/desc, filter-pill group label ("Kivistone", via a `groups` key map with `defaultValue` fallback to the raw group name) + search placeholder/clear/no-results/results-count (via `common.json`'s `catalogFilter.*`), empty/no-match states, product-card grid group heading, "View All Accessories" link (reused the existing `accessoriesPage.viewAll` key), `WhyChooseSawo` props, and `PromoBanner` props. New `kivistonePage` namespace added to `sauna.json` for en+zh. **`fi` added 2026-09-04** — full `kivistonePage` section written, 0 key-parity gap. Zero hardcoded-English hits on both audit regexes. |
| `/sauna/accessories/pails-ladles` | ✅ | ✅ | ⬜ | **Wired 2026-09-04** — `PailsLadles.jsx`, same pattern/scope as `kivistone` above. New `pailsLadlesPage` namespace in `sauna.json` (en+zh, **`fi` added 2026-09-04** — 0 key-parity gap). Group filter pills (Pails/Ladles/Pail Shower) translated via the same `groups` key-map + `defaultValue` fallback approach. Zero hardcoded-English hits on the audit regexes. |
| `/sauna/accessories/lights-covers` | ✅ | ✅ | ⬜ | **Wired 2026-09-04** — `SaunaLights.jsx`, same pattern/scope as `kivistone` above. New `lightsCoversPage` namespace in `sauna.json` (en+zh, **`fi` added 2026-09-04** — 0 key-parity gap). Group filter pills (Sauna Lights/Light Covers) translated via the same `groups` key-map + `defaultValue` fallback approach. Zero hardcoded-English hits on the audit regexes. |
| `/sauna/accessories/thermometers` | ✅ | ✅ | ⬜ | **Wired 2026-09-04** — `Thermometers.jsx`, same pattern/scope as `kivistone` above. New `thermometersPage` namespace in `sauna.json` (en+zh, **`fi` added 2026-09-04** — 0 key-parity gap). Group filter pills (Thermometers/Combined Meters/Hygrometers) translated via the same `groups` key-map + `defaultValue` fallback approach. Zero hardcoded-English hits on the audit regexes. |
| `/sauna/accessories/ventilations-add-ons` | ✅ | ✅ | ⬜ | **Wired 2026-09-04** — `VentilationsAddOns.jsx`, same pattern/scope as `kivistone` above. New `ventilationsPage` namespace in `sauna.json` (en+zh at wiring time, **`fi` added 2026-09-04** — 0 key-parity gap). Group filter pills (Ventilation/Add-Ons/Cloth Hangers) translated via the same `groups` key-map + `defaultValue` fallback approach. Zero hardcoded-English hits on the audit regexes. `CI=true npx react-scripts build` confirmed zero errors/warnings after all 5 of these pages (kivistone/pails-ladles/lights-covers/thermometers/ventilations-add-ons) were wired together — completing the full `/sauna/accessories/*` catch-all row split into per-page rows, alongside the parallel accessory-sets/benches-floor-tiles/clocks-sandtimers/doors-handles/headrests-backrests batch above. |
| `/sauna/rooms/interior-designs` | ✅ | ✅ | ⬜ | **Wired 2026-09-04** — `InteriorDesign.jsx` now calls `useLocaleT("sauna")`/`useLocalizedPath()`: SEO title/description/path/`hreflangAlternates`, hero title/subtitle, intro heading/desc, the 3 design rows (Classic/Wave/Piano Bench — title/subtitle/desc each), and the 3 Premium Wood Materials cards (Cedar/Aspen/Spruce — title/desc each). New `interiorDesignsPage` namespace added to `sauna.json` for en+zh. **`fi` added 2026-09-04** — full `interiorDesignsPage` section written (designs + materials), Cedar/Aspen/Spruce translated as Setri/Haapa/Kuusi consistent with `roomsPage.woodMaterials`; 0 key-parity gap. Zero hardcoded-English hits on both audit regexes. No internal `<Link>`s on this page to localize. |
| `/sauna/rooms/wood-panels-timbers` | ✅ | ✅ | ⬜ | **Wired 2026-09-04** — `WoodPanelandTimbers.jsx` now calls `useLocaleT("sauna")`/`useLocaleT("common")`/`useLocalizedPath()`: SEO title/description/path/`hreflangAlternates`, hero title/subtitle, brochure button text/label, intro heading/desc, "Wood Panels"/"Bench Timbers" section headings, and the 6 wood-card entries (3 panel + 3 bench, each Cedar/Aspen/Spruce — title/desc) via new `buildPanelData(t)`/`buildBenchData(t)` factory functions replacing the old static module-level `panelData`/`benchData` arrays (needed `t` at render time). Table `<th>` headers ("Profile"/"Width (mm)"/"Thickness (mm)"/"Length (mm)") translated via a shared `tableHeaders` sub-object reused by both panel and bench tables; `<td>` row data (dimension codes/numbers) left untouched. New `woodPanelsPage` namespace added to `sauna.json` for en+zh. **`fi` added 2026-09-04** — full `woodPanelsPage` section written (panels + bench + tableHeaders), Cedar/Aspen/Spruce translated as Setripuu/Haapapuu/Kuusipuu; 0 key-parity gap. Zero hardcoded-English hits on both audit regexes. No internal `<Link>`s on this page to localize. `CI=true npx react-scripts build` compiled with zero errors/warnings for both rooms pages. |
| `/sauna-heaters` (all-heaters catalog) | ✅ | ✅ | ⬜ | **Wired 2026-09-04** — `HeatersCatalog.jsx` now calls `useLocaleT("catalog")`/`useLocalizedPath()`: SEO title/description/path/`hreflangAlternates`, `CategoryHero` title/description, sidebar header ("Browse by"/"Series") and its 6 series buttons (Wall-Mounted/Tower/Stone/Floor/Combi/Dragonfire), each `CategorySection`'s `<h2>` group heading (including the Wall-Mounted brand sub-groups, whose brand names — Nordex/Mini/Scandia/etc. — stay untranslated as brand names), loading text, and the bottom CTA banner (title/desc/Contact Us/Download Catalogue buttons — `menuPaths.contact` link now correctly locale-prefixed). Product-card `<Link>`s go through `localize()`. New `catalog.json` namespace file (`heatersCatalog` key) for en+zh. **`fi` added 2026-09-04** — full `catalog.json` created (`allProducts`/`heatersCatalog`/`accessoriesCatalog`, 89/89 keys, 0 gap verified via key-diff and `npm run i18n:manifest`). Zero hardcoded-English hits on both audit regexes, `useLocaleT` count 3. |
| `/sauna-accessories` (all-accessories catalog) | ✅ | ✅ | ⬜ | **Wired 2026-09-04** — `AccessoriesCatalog.jsx` now calls `useLocaleT("catalog")`/`useLocalizedPath()`: SEO title/description/path/`hreflangAlternates`, hero eyebrow/title/description, sidebar title and its 10 category-group buttons, each `CategorySection`'s `<h2>` group heading, and loading text. (`AccessoryCard`'s own product `<Link>` was already localized before this pass.) New `catalog.json` namespace file (`accessoriesCatalog` key, shared with `heatersCatalog`/`allProducts` — see below) for en+zh. **`fi` added 2026-09-04**, same `catalog.json` pass as `/sauna-heaters` above — 0 gap. Zero hardcoded-English hits on both audit regexes, `useLocaleT` count 3. |
| `/products` (all-products catalogue) | ✅ | ⬜ | ⬜ | **Wired 2026-09-04** — `AllProducts.jsx` (the biggest of this session's 6 pages — 7 tabs: Heaters/Rooms/Sauna Controls/Steam/Infrared/Heater Accessories/Sauna Accessories, each with its own sidebar) now calls `useLocaleT("catalog")`/`useLocaleT("sauna")`/`useLocalizedPath()`: SEO title/description/path/`hreflangAlternates`, `CategoryHero` eyebrow/title/description, loading text, the 7 tab labels, every tab's sidebar header ("Browse by"/"Type" or "Categories") and its section/group buttons, every `<h2>` section heading, and all 6 empty-state messages. Product-card `<Link>`s (accessory/room/product, 3-way branch) go through `localize()`. **Gotcha found and worked around**: the section/tab/group `label` fields across `HEATER_SECTIONS`/`ROOM_SECTIONS`/`STEAM_SECTIONS`/`INFRARED_SECTIONS`/`TAB_DEFS`/`CATEGORY_SECTIONS`/`HEATER_ACCESSORIES_LABELS` double as **matching/sort keys elsewhere in this file** (`TOWER_BRAND_ORDER`, `groupByCategory`'s `labelMap`, and critically `slugify(g.label)` for sidebar-anchor DOM ids) — translating them in place would collapse multiple Chinese group labels down to the same empty/duplicate `slugify()` output (it strips non-ASCII characters), breaking the scroll-spy sidebar. Fixed by leaving every `.label` field plain English (unchanged, still used for ids/sorting/matching) and adding a `LABEL_I18N_KEYS` lookup map + `trLabel()` helper that resolves only the *displayed* text through `catalog.json`, falling back to the raw English label for anything unmapped (e.g. the "Other" catch-all bucket). The Sauna Controls tab's group labels (Coming Soon/Saunova Series/Innova Series/etc.) reuse `sauna.json`'s existing `controlsPage.groupLabels` via a separate `trControlsGroupLabel()` helper instead of duplicating them in `catalog.json`. New `catalog.json` namespace file (`allProducts` key) for en+zh (`fi` left ⬜). Zero hardcoded-English hits on both audit regexes, `useLocaleT` count 3. Product *names* themselves (rendered in `ProductCard`) are unaffected — that's the separate Supabase `product_translations` pipeline, out of scope here. `CI=true npx react-scripts build` confirmed zero errors after all 3 of these catalogue pages (`/sauna-heaters`, `/sauna-accessories`, `/products`) were wired together. |
...
## Product catalogue pages section (2026-09-04)

Session goal: `zh` wiring for the remaining sauna heater sub-pages (Floor/
Combi/Dragonfire) plus the three full product-catalogue pages (`/products`,
`/sauna-heaters`, `/sauna-accessories`) — all 6 confirmed at 0 `useLocaleT`
hits before starting. `fi` was **not** touched for any of these —
`zh`-only per this session's established convention, `fi` remains ⬜
throughout. This session ran concurrently with another workstream wiring
the `/sauna/accessories/*` and `/sauna/rooms/*` sub-pages (see their rows
above) — both sessions edited `sauna.json` at the same time; the
`floorPage`/`combiPage`/`dragonfirePage` namespace additions from this
session and that session's own additions landed together in a single
commit (`c8471885`) as a result, not a separate commit — the content
itself is unaffected, just the commit attribution.

- **Floor/Combi/Dragonfire** (`/sauna/heaters/floor|combi|dragonfire`):
  unlike Stone/WallMounted/Tower — each of which turned out to already be
  partially wired when a previous session reached it — these three had
  **zero** prior i18n wiring: no `useLocaleT`/`useLocalizedPath` imports,
  hardcoded SEO props, hardcoded hero/intro/why/promo copy, and two
  locale-dropping `<Link>`s each (the product-card link and the "View All
  Heaters" link, both bare English paths). Wired all three from scratch
  following the `Stone.jsx` pattern exactly. Three new `sauna.json`
  namespaces (`floorPage`/`combiPage`/`dragonfirePage`), en+zh only.
- **`/sauna-heaters`, `/sauna-accessories`, `/products`**: none of the
  three had any i18n wiring — `HeatersCatalog.jsx`/`AccessoriesCatalog.jsx`
  were flagged as untouched in this file already; `AllProducts.jsx`
  (`/products`) wasn't tracked in any table at all before this session.
  New shared `catalog.json` namespace file (`allProducts`/`heatersCatalog`/
  `accessoriesCatalog` keys), en+zh only, checked first against
  `sauna.json`'s existing page-namespace precedent to confirm no existing
  file already fit before creating a new one.
- **`AllProducts.jsx` label/id collision, found and worked around** (see
  its own row above for the full mechanic): several of this page's group/
  section `label` strings are reused as `slugify()` inputs to generate
  sidebar-anchor DOM ids elsewhere in the same file. Translating those
  strings in place would have silently broken the scroll-spy sidebar for
  `zh` visitors (Chinese characters strip to nothing under `slugify()`,
  so multiple groups could collapse onto the same empty id). Solved with a
  separate id-vs-display-text split (`LABEL_I18N_KEYS` + `trLabel()`)
  rather than changing the underlying data shape — worth checking for the
  same pattern (`slugify(something.label)` or similar) before translating
  any other page that builds its own scroll-spy sidebar from a labelled
  section list.
- **Build-blocking pre-existing issue found, not caused by this session**:
  a `CI=true` build run mid-session failed on an unrelated file
  (`Thermometers.jsx`, then a different file on the next run,
  `VentilationsAddOns.jsx`, then `WoodPanelandTimbers.jsx`) each time with
  `'tc' is assigned a value but never used` / `'tc' is not defined` —
  caused by the other concurrent workstream's in-progress edits to those
  exact files being captured mid-save by the build. Not this session's
  bug and not fixed by this session (chasing it would have meant editing
  files another session owned mid-edit); the build was green
  ("Compiled successfully") once re-run after that other workstream's
  edits had settled. Worth knowing about if a `CI=true` build intermittently
  fails on a file nobody in the current session touched — check whether
  another session/process has uncommitted changes to that exact file
  before assuming it's a real regression.
- Every page audited with the standard process: 3-word/2-word
  hardcoded-JSX-text regex + `alt|title|placeholder|aria-label="..."`
  attribute regex (zero real hits on all 6 files), `grep -c useLocaleT`
  = 3 in every file, `npm run i18n:manifest` confirms `catalog: zh
...
Privacy Policy, Sitemap, NotFound (404). None of the three has any `t()`
wiring yet in their `.jsx` — `PrivacyPolicy.jsx`/`Sitemap.jsx`/`NotFound.jsx`
still render hardcoded English regardless of locale. **Translation JSON now
exists for all three** (see the "Legal / Utility pages" table below) —
`en`/`zh` were already present from an earlier session, `fi` added
2026-09-04 as a pure-JSON pass (no `.jsx` touched, per that task's scope) —
but none of it is actually read by the page yet. Wiring these three (adding
`useLocaleT`/`useLocalizedPath` calls to the components, following the
`Stone.jsx`/`AboutUs/*` pattern used everywhere else in this file) is still
open work.
...
| Route | Wired | FI written | ZH written | Live | Notes |
|---|---|---|---|---|---|
| `/privacy` | ⬜ | ✅ | ✅ | ⬜ | `privacy.json` — `meta`/`hero`/`footer` plus an 8-item `sections` array (some with `subsections`, `bullets`, or `email`, matching the English source's mixed shape exactly). `en`/`zh` pre-existing; **`fi` added 2026-09-04**, translated independently from the English source (not derived from `zh`), formal/precise legal register, 65/65 keys (including full array-item structure) verified 0 gap via key-diff and `npm run i18n:manifest`. `PrivacyPolicy.jsx` itself has no `t()` wiring yet — JSON is ready, component isn't reading it. |
| `/sitemap` | ⬜ | ✅ | ✅ | ⬜ | `sitemap.json` — nav labels plus a `productGroups` map. `en`/`zh` pre-existing; **`fi` added 2026-09-04**, 87/87 keys, 0 gap. Per spec, `productGroups`' *keys* were left as the literal original English strings (e.g. `"Sauna Heaters"` stays `"Sauna Heaters"` as a key) since they're used as lookup keys elsewhere — only the values were translated. `Sitemap.jsx` itself has no `t()` wiring yet. |
| `/404` (NotFound) | ⬜ | ✅ | ✅ | ⬜ | `notfound.json` — heading/desc/help-box/popular-sections copy. `en`/`zh` pre-existing; **`fi` added 2026-09-04**, 20/20 keys, 0 gap. `NotFound.jsx` itself has no `t()` wiring yet. |

Also fixed the same session: `common.json`'s `accessoryCard` block (5 keys —
`bestSeller`/`close`/`watchVideo`/`code`/`option`) existed in `en`/`zh` but
was missing from `fi` entirely (a pre-existing gap, not introduced by this
pass) — added, translated independently, verified 0 key gap against `en`
via key-diff. `npm run i18n:manifest` confirms `fi: translated` for
`common` (the remaining `de: stale` result for `common` is unrelated,
...
2. Wire `PrivacyPolicy.jsx`/`Sitemap.jsx`/`NotFound.jsx` to actually call
   `t()` against their now-complete `en`/`fi`/`zh` JSON (see "Legal /
   Utility pages" above) — translation content is ready, the JSX wiring
   itself is the only remaining gap for these three.
