...
| `/sauna/accessories/kivistone` | ✅ | ⬜ | ⬜ | **Wired 2026-09-04** — `Kivistone.jsx` now calls `useLocaleT("sauna")`/`useLocaleT("common")`/`useLocalizedPath()` (same `Stone.jsx`/`SaunaAccessories.jsx` pattern as the 5-page batch above, done concurrently by a parallel workstream in this same repo): SEO title/description/path/`hreflangAlternates`, hero title/subtitle/alt/brochure button, intro heading/desc, filter-pill group label ("Kivistone", via a `groups` key map with `defaultValue` fallback to the raw group name) + search placeholder/clear/no-results/results-count (via `common.json`'s `catalogFilter.*`), empty/no-match states, product-card grid group heading, "View All Accessories" link (reused the existing `accessoriesPage.viewAll` key), `WhyChooseSawo` props, and `PromoBanner` props. New `kivistonePage` namespace added to `sauna.json` for en+zh (`fi` intentionally left ⬜, out of scope). Zero hardcoded-English hits on both audit regexes. |
| `/sauna/accessories/pails-ladles` | ✅ | ⬜ | ⬜ | **Wired 2026-09-04** — `PailsLadles.jsx`, same pattern/scope as `kivistone` above. New `pailsLadlesPage` namespace in `sauna.json` (en+zh only). Group filter pills (Pails/Ladles/Pail Shower) translated via the same `groups` key-map + `defaultValue` fallback approach. Zero hardcoded-English hits on the audit regexes. |
...
| `/sauna/accessories/thermometers` | ✅ | ⬜ | ⬜ | **Wired 2026-09-04** — `Thermometers.jsx`, same pattern/scope as `kivistone` above. New `thermometersPage` namespace in `sauna.json` (en+zh only). Group filter pills (Thermometers/Combined Meters/Hygrometers) translated via the same `groups` key-map + `defaultValue` fallback approach. Zero hardcoded-English hits on the audit regexes. |
