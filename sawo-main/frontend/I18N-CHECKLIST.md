...
**Update 2026-09-25:** `fi` key parity re-verified for `stonePage`, `wallMountedPage`,
`towerPage` and `catalog.allProducts` (0 missing), so those rows' "FI written"
cells are now ✅ (they read ⬜ from when only `zh` existed). `de`, `fr`, `es`,
`th` and `ja` are also complete on both tracks (383/382 `product_translations`
rows per locale, 0 stale); none is native-reviewed or in `TRANSLATED_PATHS`.
Twelve `fi` Nordex/Minidragon/Scandifire product names had `Black` untranslated —
now `Musta`, matching the rest of the catalogue. The old per-page heater
`*Page.promo.*` keys are already gone; the accessory pages and `controlsPage`
still use theirs. `CI=true npm run build` + prerender pass.
...
| `/sauna/accessories/benches-floor-tiles` | ✅ | ✅ | ⬜ | **Wired 2026-09-04** — `BenchesFloorTiles.jsx`, same pattern/scope as `accessory-sets` above. New `benchesFloorTilesPage` namespace in `sauna.json` (en+zh only). Group filter pills (Benches/Wooden Floor Mats/Cloth Hangers) translated via the same `groups` key-map + `defaultValue` fallback approach. Zero hardcoded-English hits on the audit regexes. **`fi` added 2026-09-04** — full `benchesFloorTilesPage` section written, reusing established `accessoriesPage` Finnish accessory-catalog terminology; key-parity verified 0 gap. |
| `/sauna/accessories/clocks-sandtimers` | ✅ | ✅ | ⬜ | **Wired 2026-09-04** — `ClocksSandtimers.jsx`, same pattern/scope as `accessory-sets` above. New `clocksSandtimersPage` namespace in `sauna.json` (en+zh only). Group filter pills (Clocks/Sand Timers) translated via the same `groups` key-map + `defaultValue` fallback approach. Zero hardcoded-English hits on the audit regexes. **`fi` added 2026-09-04** — full `clocksSandtimersPage` section written, reusing established `accessoriesPage` Finnish accessory-catalog terminology; key-parity verified 0 gap. |
| `/sauna/accessories/doors-handles` | ✅ | ✅ | ⬜ | **Wired 2026-09-04** — `DoorsHandles.jsx`, same pattern/scope as `accessory-sets` above. New `doorsHandlesPage` namespace in `sauna.json` (en+zh only). Group filter pills (Sauna Doors/Handles) translated via the same `groups` key-map + `defaultValue` fallback approach — the underlying `GROUP_KEYWORDS`/name-matching logic (which relies on the untranslated English "Door"/"Door Handle" substrings) was left completely untouched, only the *displayed* pill/heading text goes through `t()`. Zero hardcoded-English hits on the audit regexes. **`fi` added 2026-09-04** — full `doorsHandlesPage` section written, reusing established `accessoriesPage` Finnish accessory-catalog terminology; key-parity verified 0 gap. |
| `/sauna/accessories/headrests-backrests` | ✅ | ✅ | ⬜ | **Wired 2026-09-04** — `HeadrestsBackrests.jsx`, same pattern/scope as `accessory-sets` above. New `headrestsBackrestsPage` namespace in `sauna.json` (en+zh only). Group filter pills (Headrests/Backrests) translated via the same `groups` key-map + `defaultValue` fallback approach. Zero hardcoded-English hits on the audit regexes. `CI=true npx react-scripts build` confirmed zero errors after all 5 of these pages were wired together. `npm run i18n:manifest` confirms `sauna: zh translated`, 0 gap (fi shows more keys behind source than before, expected — these 5 pages' `fi` strings were intentionally not added). **`fi` added 2026-09-04** — full `headrestsBackrestsPage` section written, reusing established `accessoriesPage` Finnish accessory-catalog terminology; key-parity verified 0 gap. |
| `/sauna/accessories/kivistone` | ✅ | ✅ | ⬜ | **Wired 2026-09-04** — `Kivistone.jsx` now calls `useLocaleT("sauna")`/`useLocaleT("common")`/`useLocalizedPath()` (same `Stone.jsx`/`SaunaAccessories.jsx` pattern as the 5-page batch above, done concurrently by a parallel workstream in this same repo): SEO title/description/path/`hreflangAlternates`, hero title/subtitle/alt/brochure button, intro heading/desc, filter-pill group label ("Kivistone", via a `groups` key map with `defaultValue` fallback to the raw group name) + search placeholder/clear/no-results/results-count (via `common.json`'s `catalogFilter.*`), empty/no-match states, product-card grid group heading, "View All Accessories" link (reused the existing `accessoriesPage.viewAll` key), `WhyChooseSawo` props, and `PromoBanner` props. New `kivistonePage` namespace added to `sauna.json` for en+zh. **`fi` added 2026-09-04** — full `kivistonePage` section (incl. `search.placeholder`) written, reusing established `accessoriesPage` Finnish accessory-catalog terminology; key-parity verified 0 gap. Zero hardcoded-English hits on both audit regexes. |
| `/sauna/accessories/pails-ladles` | ✅ | ✅ | ⬜ | **Wired 2026-09-04** — `PailsLadles.jsx`, same pattern/scope as `kivistone` above. New `pailsLadlesPage` namespace in `sauna.json` (en+zh only). Group filter pills (Pails/Ladles/Pail Shower) translated via the same `groups` key-map + `defaultValue` fallback approach. Zero hardcoded-English hits on the audit regexes. **`fi` added 2026-09-04** — full `pailsLadlesPage` section (incl. `search.placeholder`) written, reusing established `accessoriesPage` Finnish accessory-catalog terminology; key-parity verified 0 gap. |
| `/sauna/accessories/lights-covers` | ✅ | ✅ | ⬜ | **Wired 2026-09-04** — `SaunaLights.jsx`, same pattern/scope as `kivistone` above. New `lightsCoversPage` namespace in `sauna.json` (en+zh only). Group filter pills (Sauna Lights/Light Covers) translated via the same `groups` key-map + `defaultValue` fallback approach. Zero hardcoded-English hits on the audit regexes. **`fi` added 2026-09-04** — full `lightsCoversPage` section (incl. `search.placeholder`) written, reusing established `accessoriesPage` Finnish accessory-catalog terminology; key-parity verified 0 gap. |
| `/sauna/accessories/thermometers` | ✅ | ✅ | ⬜ | **Wired 2026-09-04** — `Thermometers.jsx`, same pattern/scope as `kivistone` above. New `thermometersPage` namespace in `sauna.json` (en+zh only). Group filter pills (Thermometers/Combined Meters/Hygrometers) translated via the same `groups` key-map + `defaultValue` fallback approach. Zero hardcoded-English hits on the audit regexes. **`fi` added 2026-09-04** — full `thermometersPage` section (incl. `search.placeholder`) written, reusing established `accessoriesPage` Finnish accessory-catalog terminology; key-parity verified 0 gap. |
| `/sauna/accessories/ventilations-add-ons` | ✅ | ✅ | ⬜ | **Wired 2026-09-04** — `VentilationsAddOns.jsx`, same pattern/scope as `kivistone` above. New `ventilationsPage` namespace in `sauna.json` (en+zh only). Group filter pills (Ventilation/Add-Ons/Cloth Hangers) translated via the same `groups` key-map + `defaultValue` fallback approach. Zero hardcoded-English hits on the audit regexes. `CI=true npx react-scripts build` confirmed zero errors/warnings after all 5 of these pages (kivistone/pails-ladles/lights-covers/thermometers/ventilations-add-ons) were wired together — completing the full `/sauna/accessories/*` catch-all row split into per-page rows, alongside the parallel accessory-sets/benches-floor-tiles/clocks-sandtimers/doors-handles/headrests-backrests batch above. **`fi` added 2026-09-04** — full `ventilationsPage` section (incl. `search.placeholder`) written, reusing established `accessoriesPage` Finnish accessory-catalog terminology; key-parity verified 0 gap. |
| `/sauna/rooms/interior-designs` | ✅ | ✅ | ⬜ | **Wired 2026-09-04** — `InteriorDesign.jsx` now calls `useLocaleT("sauna")`/`useLocalizedPath()`: SEO title/description/path/`hreflangAlternates`, hero title/subtitle, intro heading/desc, the 3 design rows (Classic/Wave/Piano Bench — title/subtitle/desc each), and the 3 Premium Wood Materials cards (Cedar/Aspen/Spruce — title/desc each). New `interiorDesignsPage` namespace added to `sauna.json` for en+zh. Zero hardcoded-English hits on both audit regexes. No internal `<Link>`s on this page to localize. **`fi` added 2026-09-04** — full `interiorDesignsPage` section written (Cedar/Aspen/Spruce → Setri/Haapa/Kuusi, matching the established `MATERIAL_WORD_DICTIONARY` from the product-translation batches); key-parity verified 0 gap. |
| `/sauna/rooms/wood-panels-timbers` | ✅ | ✅ | ⬜ | **Wired 2026-09-04** — `WoodPanelandTimbers.jsx` now calls `useLocaleT("sauna")`/`useLocaleT("common")`/`useLocalizedPath()`: SEO title/description/path/`hreflangAlternates`, hero title/subtitle, brochure button text/label, intro heading/desc, "Wood Panels"/"Bench Timbers" section headings, and the 6 wood-card entries (3 panel + 3 bench, each Cedar/Aspen/Spruce — title/desc) via new `buildPanelData(t)`/`buildBenchData(t)` factory functions replacing the old static module-level `panelData`/`benchData` arrays (needed `t` at render time). Table `<th>` headers ("Profile"/"Width (mm)"/"Thickness (mm)"/"Length (mm)") translated via a shared `tableHeaders` sub-object reused by both panel and bench tables; `<td>` row data (dimension codes/numbers) left untouched. New `woodPanelsPage` namespace added to `sauna.json` for en+zh. Zero hardcoded-English hits on both audit regexes. No internal `<Link>`s on this page to localize. `CI=true npx react-scripts build` compiled with zero errors/warnings for both rooms pages. **`fi` added 2026-09-04** — full `woodPanelsPage` section written (table headers → Profiili/Leveys (mm)/Paksuus (mm)/Pituus (mm), Cedar/Aspen/Spruce → Setri/Haapa/Kuusi); key-parity verified 0 gap. |
