...
const instructions = {
  ...
  needsReview:
    "Every entry the scanner flagged has already been hand-reviewed against its actual source file (see scripts/i18n/resolutions.js) — real copy was reconstructed into full sentences (inline <b>/<strong>-split fragments) and promoted into `pages`/`shared` below; everything still listed in the top-level `needsReview` array has been CONFIRMED non-copy, each with a `reason` explaining why (decorative icon glyph, dash/asterisk placeholder, email address, URL, product/model code, internal classification keyword, etc.). Nothing in `needsReview` needs translating — it's a documented-exclusion list, not an ambiguous one. Do not translate these values.",
  preserveExactly:
