 ...
 * i18n.changeLanguage race). BUT the Finnish catalog itself is now a lazy
 * webpack chunk (see i18n.js's loadLocale()) fetched after first paint, not
 * bundled eagerly — so waitFor explicitly waits for the real Finnish text
 * (same "Koe" marker sanityCheck already trusts) rather than a flat sleep,
 * which would otherwise race the chunk fetch and could bake the English
 * fallback into this snapshot instead of failing loudly.
 */
module.exports = {
  path: "/fi",
  ...
  async waitFor(page) {
    ...
    await page.waitForFunction(
      () => document.getElementById("root")?.innerHTML.includes("Koe"),
      { timeout: 30000 }
    );
