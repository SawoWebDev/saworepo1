 ...
 * i18n.changeLanguage race). BUT the German catalog itself is now a lazy
 * webpack chunk (see i18n.js's loadLocale()) fetched after first paint, not
 * bundled eagerly — so waitFor explicitly waits for the real German text
 * (same "Erleben Sie" marker sanityCheck already trusts) rather than a flat
...
module.exports = {
  ...
  async waitFor(page) {
    ...
    await page.waitForFunction(
      () => document.getElementById("root")?.innerHTML.includes("Erleben Sie"),
      { timeout: 30000 }
    );
