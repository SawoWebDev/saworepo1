...
export default function HeadrestsBackrests() {
  ...
  return (
    <div className="relative">
      <SEO
        title={t("headrestsBackrestsPage.meta.title")}
        description={t("headrestsBackrestsPage.meta.description")}
        path={localize("/sauna/accessories/headrests-backrests")}
        hreflangAlternates={{ en: "/sauna/accessories/headrests-backrests", zh: "/zh/sauna/accessories/headrests-backrests" }}
      />
      <style>{`
      ...
      <section className="relative isolate min-h-[95vh] flex flex-col justify-center items-center text-center px-6" style={{ backgroundColor: "#241c17" }}>
        <img
          src={heroImg}
          alt={t("headrestsBackrestsPage.hero.alt")}
        ...
        <div className="relative z-10">
          <h1 className="wm-hero-title">{t("headrestsBackrestsPage.hero.title")}</h1>
          <p className="wm-hero-subtitle">{t("headrestsBackrestsPage.hero.subtitle")}</p>
      ...
      <section className="wm-section">
        <div className="wm-container text-center">
          <h2 className="wm-products-title">{t("headrestsBackrestsPage.intro.title")}</h2>
          <p className="wm-products-desc">{t("headrestsBackrestsPage.intro.desc")}</p>
        </div>
      </section>
          ...
          <div className="wm-container">
            <div className="wm-filter-search-row">
              <div className="wm-filter-pills-group">
                ...
                {groupNames.map(g => (
                  <button key={g} className={`wm-filter-btn ${activeGroup === g ? "wm-filter-btn--active" : ""}`} onClick={() => setActiveGroup(g)}>{t(`headrestsBackrestsPage.groups.${g}`, { defaultValue: g })}</button>
                ))}
              </div>
              <div className="wm-search-wrap wm-search-bar-fixed">
                <i className="fa-solid fa-magnifying-glass wm-search-icon" />
                <input className="wm-search-input" type="text" value={search} onChange={e => setSearch(e.target.value)} placeholder={tc("catalogFilter.searchPlaceholder", { category: t("headrestsBackrestsPage.meta.title") })} />
            ...
            <>
              {visibleGroups.length === 0 ? (
                <div style={{ textAlign: "center", padding: "48px 0", fontFamily: "'Montserrat', sans-serif", color: "#a67853" }}>
                  ...
                  <p style={{ margin: 0 }}>{tc("catalogFilter.noMatch", { category: t("headrestsBackrestsPage.meta.title"), query: search })}</p>
                ...
                visibleGroups.map(group => {
                  ...
                  return (
                    <div className="wm-group" key={group}>
                      <h3 className="wm-group-title">{t(`headrestsBackrestsPage.groups.${group}`, { defaultValue: group }).toUpperCase()}</h3>
      ...
      <WhyChooseSawo
        eyebrow={t("headrestsBackrestsPage.why.eyebrow")}
        title={t("headrestsBackrestsPage.why.title")}
        description={t("headrestsBackrestsPage.why.description")}
        brochureHref="https://www.sawo.com/wp-content/uploads/2026/07/SAWO-Sauna-Accessories-2026.pdf"
        brochureLabel={tc("viewBrochure")}
      />

      <PromoBanner
        title={t("headrestsBackrestsPage.promo.title")}
        subtitle={t("headrestsBackrestsPage.promo.subtitle")}
