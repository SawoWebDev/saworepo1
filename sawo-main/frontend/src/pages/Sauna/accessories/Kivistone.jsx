...
export default function Kivistone() {
  ...
  return (
    <div className="relative">
      <SEO
        title={t("kivistonePage.meta.title")}
        description={t("kivistonePage.meta.description")}
        path={localize("/sauna/accessories/kivistone")}
        hreflangAlternates={{ en: "/sauna/accessories/kivistone", zh: "/zh/sauna/accessories/kivistone" }}
      />
      <style>{`
      ...
      <section className="relative isolate min-h-[95vh] flex flex-col justify-center items-center text-center px-6" style={{ backgroundColor: "#241c17" }}>
        ...
        <div className="relative z-10">
          <h1 className="wm-hero-title">{t("kivistonePage.hero.title")}</h1>
          <p className="wm-hero-subtitle">{t("kivistonePage.hero.subtitle")}</p>
          <div style={{ marginTop: "32px" }}>
            <BrochureDropdownButton text={tc("viewBrochure")} href="https://www.sawo.com/wp-content/uploads/2026/06/Kivistone-Brochure_-2026.pdf" />
          </div>
        </div>
      <HeroWave />
      </section>
      ...
      <section className="wm-section">
        <div className="wm-container text-center">
          <h2 className="wm-products-title">{t("kivistonePage.intro.title")}</h2>
          <p className="wm-products-desc">{t("kivistonePage.intro.desc")}</p>
        </div>
      </section>
            ...
            <div className="wm-filter-search-row">
              {groupNames.length > 1 && (
                <div className="wm-filter-pills-group">
                  ...
                  {groupNames.map(g => (
                    <button key={g} className={`wm-filter-btn ${activeGroup === g ? "wm-filter-btn--active" : ""}`} onClick={() => setActiveGroup(g)}>{t(`kivistonePage.groups.${g}`, { defaultValue: g })}</button>
                  ))}
                </div>
              )}
              <div className="wm-search-wrap wm-search-bar-fixed">
                <i className="fa-solid fa-magnifying-glass wm-search-icon" />
                <input className="wm-search-input" type="text" value={search} onChange={e => setSearch(e.target.value)} placeholder={t("kivistonePage.search.placeholder")} />
            ...
            <>
              {visibleGroups.length === 0 ? (
                <div style={{ textAlign: "center", padding: "48px 0", fontFamily: "'Montserrat', sans-serif", color: "#a67853" }}>
                  ...
                  <p style={{ margin: 0 }}>{tc("catalogFilter.noMatch", { category: t("kivistonePage.meta.title"), query: search })}</p>
                ...
                visibleGroups.map(group => {
                  ...
                  return (
                    <div className="wm-group" key={group}>
                      <h3 className="wm-group-title">{t(`kivistonePage.groups.${group}`, { defaultValue: group }).toUpperCase()}</h3>
      ...
      <WhyChooseSawo
        eyebrow={t("kivistonePage.why.eyebrow")}
        title={t("kivistonePage.why.title")}
        description={t("kivistonePage.why.description")}
        brochureHref="https://www.sawo.com/wp-content/uploads/2026/06/Kivistone-Brochure_-2026.pdf"
        brochureLabel={tc("viewBrochure")}
      />

      <PromoBanner
        title={t("kivistonePage.promo.title")}
        subtitle={t("kivistonePage.promo.subtitle")}
