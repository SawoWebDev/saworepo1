// Fill engine for the German (de) product-translation push.
//
// Reads every `fill-de-data-*.mjs` module next to this file (each default-exports
// a plain object: exact English string -> German string), then walks the already
// extracted `<slug>.de.packet.json` packets and replaces every prose string that has a
...
//   node fill-de-apply.mjs <slugs.txt | slug1,slug2,...>   fill + report missing
//   node fill-de-apply.mjs --all                             every de packet on disk
...
// overwrites the packet, keep a pristine copy elsewhere and point DE_SRC at it to re-run
// the fill after editing the dictionary (idempotent that way).
const SRC_DIR = process.env.DE_SRC || DATA_DIR;
...
for (const f of fs.readdirSync(__dirname).filter((n) => /^fill-de-data-.*\.mjs$/.test(n)).sort()) {
...
function translateDescription(html, missing) {
  ...
  const flush = (text) => {
    ...
    if (t !== undefined) {
      // a translation that starts with punctuation ("-Funktion", ", der ...") attaches to the previous node
      const l = /^[,.;:!?)\]-]/.test(t) ? "" : lead;
      return l + t + trail;
    }
...
const MATERIAL_DONE_RE = /^((Zeder|Espe|Hemlock|Erle|Kiefer|Fichte|Birke|Schwarz|Weiß|Grau|Silber|Naturfarben|Aluminium|Schwarzmetall)( \(.+\))?|\(.+\))$/;
...
function tr(value, missing, label, { html = false } = {}) {
  ...
  // already filled by extract via product-i18n.js MATERIAL_WORD_DICTIONARY ("Zeder (513-D)")
...
if (process.argv[1] && fileURLToPath(import.meta.url) === path.resolve(process.argv[1])) {
  ...
  if (args.includes("--all")) {
    slugs = fs.readdirSync(DATA_DIR).filter((f) => f.endsWith(".de.packet.json")).map((f) => f.replace(".de.packet.json", ""));
  } else if (arg && fs.existsSync(arg)) {
    slugs = fs.readFileSync(arg, "utf8").split(/\r?\n/).map((s) => s.trim()).filter(Boolean);
  } else if (arg) {
    slugs = arg.split(",").map((s) => s.trim()).filter(Boolean);
  } else {
    console.error("usage: node fill-de-apply.mjs <slugs.txt|a,b,c|--all> [--write-nothing]");
    process.exit(1);
  }
  const allMissing = new Map();
  let written = 0;
  for (const slug of slugs) {
    const file = path.join(DATA_DIR, `${slug}.de.packet.json`);
    if (!fs.existsSync(file)) {
      console.error(`no packet: ${slug}`);
      continue;
    }
    const srcFile = path.join(SRC_DIR, `${slug}.de.packet.json`);
  ...
  console.log(`${allMissing.size} distinct string(s) still without a German entry:`);
